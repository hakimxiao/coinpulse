...
const CandleStickChart = ({
   ...
   * - period tetap dipakai via days
   */
  const fetchOHLCData = async (selectedPeriod: Period) => {
    try {
      setLoading(true);

      let { days } = PERIOD_CONFIG[selectedPeriod];

      /**
       * ===============================
       * FIX KHUSUS UNTUK PERIOD "max"
       * ===============================
       * /ohlc TIDAK mendukung days = 'max'
       */
      if (selectedPeriod === 'max') {
        days = 365; // safest upper bound for OHLC
      }
  ...
  /**
   * ===============================
   * UPDATE DATA SAAT PERIOD / DATA BERUBAH
   * ===============================
   */
  useEffect(() => {
